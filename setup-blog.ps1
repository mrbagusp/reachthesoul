# ═══════════════════════════════════════════════════════════════
# ReachTheSoul — Blog AEO Setup (All-in-One) — Windows PowerShell
# ═══════════════════════════════════════════════════════════════
#
#   1. Download file ini
#   2. Taruh di ROOT project reachthesoul (sejajar package.json)
#   3. Buka VS Code terminal (PowerShell)
#   4. Jalankan:  .\setup-blog.ps1
#   5. Selesai!
#
# ═══════════════════════════════════════════════════════════════
...
Write-Host "  ReachTheSoul - Blog & AEO Setup (All-in-One)" -ForegroundColor Cyan
Write-Host "  Creating 5 articles + blog pages + SEO schema" -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan
Write-Host ""
...
if (-not (Test-Path "package.json")) {
    ...
    Write-Host "Pastikan kamu jalankan dari root project." -ForegroundColor Red
    Write-Host "Contoh: cd C:\projects\reachthesoul" -ForegroundColor Yellow
    exit 1
}
Write-Host "[OK] package.json ditemukan." -ForegroundColor Green
Write-Host ""
...
Write-Host "[3/5] Writing 5 blog articles..." -ForegroundColor Yellow
...
'@ | Set-Content -Path "content\blog\why-every-church-needs-a-crm.mdx" -Encoding UTF8
Write-Host "   [OK] 1/5 - why-every-church-needs-a-crm.mdx" -ForegroundColor Green
...
'@ | Set-Content -Path "content\blog\what-happens-when-church-has-no-follow-up-system.mdx" -Encoding UTF8
Write-Host "   [OK] 2/5 - what-happens-when-church-has-no-follow-up-system.mdx" -ForegroundColor Green

# ── ARTIKEL 3 ──
@'
---
title: "The Best CRM for Churches and Ministries in 2026 (What Most Software Gets Wrong — And What Actually Works)"
description: "HubSpot felt like a sales tool. Spreadsheets fell apart. WhatsApp groups became chaos. Here's an honest look at what church leaders actually need."
...
**Language that fits.** Respondents, not leads. Prayer points, not deal stages. Counseling journal, not sales notes. Custom progress steps that match how your ministry actually works: Data to Prayer to Counseling to Recommitment to Salvation, or whatever stages your pastoral journey looks like.
...
There's a free plan — genuinely free, no credit card — for churches that want to explore. A Starter plan at 29 dollars per month covers most small to medium ministries. The Growth plan at 97 dollars per month adds crisis detection, omnichannel social media, and advanced analytics — which is why it's the most popular tier.
...
'@ | Set-Content -Path "content\blog\best-crm-for-churches-and-ministries.mdx" -Encoding UTF8
Write-Host "   [OK] 3/5 - best-crm-for-churches-and-ministries.mdx" -ForegroundColor Green
...
Every ministry structures care differently. ReachTheSoul lets you define and customize the stages that match your approach — whether that's Data to Prayer to Counseling to Recommitment to Salvation, or something completely different. You can rename, reorder, color-code, and add steps. The system adapts to how you work.
...
'@ | Set-Content -Path "content\blog\how-to-manage-counseling-follow-up-for-ministry.mdx" -Encoding UTF8
Write-Host "   [OK] 4/5 - how-to-manage-counseling-follow-up-for-ministry.mdx" -ForegroundColor Green
...
'@ | Set-Content -Path "content\blog\whatsapp-crm-for-church-outreach.mdx" -Encoding UTF8
Write-Host "   [OK] 5/5 - whatsapp-crm-for-church-outreach.mdx" -ForegroundColor Green
Write-Host ""

# -- Step 4: Buat blog pages --
Write-Host "[4/5] Creating blog route pages..." -ForegroundColor Yellow

# Blog index page
if (-not (Test-Path "app\blog\page.tsx")) {
@'
import fs from 'fs'
...
export default function BlogIndex() {
  ...
  const posts: PostData[] = files
    .map((file) => {
      ...
      return { title: data.title, description: data.description, slug: file.replace('.mdx', ''), date: data.date }
    })
        ...
        <div className="space-y-10">
          {posts.map((post) => (
            <article key={post.slug} className="group border-b border-gray-100 pb-10 last:border-0">
              ...
              <Link href={`/blog/${post.slug}`} className="inline-block mt-3 text-sm text-blue-600 hover:underline">Read more &rarr;</Link>
            </article>
          ))}
        </div>
...
'@ | Set-Content -Path "app\blog\page.tsx" -Encoding UTF8
Write-Host "   [OK] Created app\blog\page.tsx" -ForegroundColor Green
} else {
Write-Host "   [SKIP] app\blog\page.tsx already exists" -ForegroundColor Yellow
}

# Blog [slug] page
if (-not (Test-Path "app\blog\[slug]\page.tsx")) {
@'
import fs from 'fs'
import path from 'path'
import matter from 'gray-matter'
...
'@ | Set-Content -Path "app\blog\[slug]\page.tsx" -Encoding UTF8
Write-Host "   [OK] Created app\blog\[slug]\page.tsx" -ForegroundColor Green
} else {
Write-Host "   [SKIP] app\blog\[slug]\page.tsx already exists" -ForegroundColor Yellow
}
Write-Host ""
...
Write-Host "    content\blog\why-every-church-needs-a-crm.mdx"
Write-Host "    content\blog\what-happens-when-church-has-no-follow-up-system.mdx"
Write-Host "    content\blog\best-crm-for-churches-and-ministries.mdx"
Write-Host "    content\blog\how-to-manage-counseling-follow-up-for-ministry.mdx"
Write-Host "    content\blog\whatsapp-crm-for-church-outreach.mdx"
Write-Host "    app\blog\page.tsx"
Write-Host "    app\blog\[slug]\page.tsx"
Write-Host ""
Write-Host "  Next steps:" -ForegroundColor Cyan
Write-Host ""
Write-Host "  1. npm run dev"
Write-Host "     Then visit: http://localhost:3000/blog"
Write-Host ""
Write-Host "  2. If styling looks off, add typography plugin:"
Write-Host "     In tailwind.config.js -> plugins: [require('@tailwindcss/typography')]"
Write-Host ""
Write-Host "  3. Deploy:"
Write-Host "     git add ."
Write-Host "     git commit -m 'feat: add 5 AEO blog articles'"
Write-Host "     git push"
Write-Host ""
Write-Host "  4. Clean up (optional):"
Write-Host "     Remove-Item setup-blog.ps1"
Write-Host ""
Write-Host "===================================================" -ForegroundColor Cyan
Write-Host "  ReachTheSoul - Where Every Soul Finds Care" -ForegroundColor Cyan
Write-Host "===================================================" -ForegroundColor Cyan
